...
namespace NBitcoin.Protocol
{
	...
	public class AddressManager : IBitcoinSerializable
	{
		internal class AddressInfo : IBitcoinSerializable
		{
			...
			private byte[] source = new byte[16];
		...
		private byte nVersion = 1;
		private byte nKeySize = 32;
		internal uint256 nKey;
		internal int nNew;
		internal int nTried;
		private List<int> vRandom;

		private int[,] vvNew;
		private int[,] vvTried;

		private Dictionary<int, AddressInfo> mapInfo = new Dictionary<int, AddressInfo>();
		private Dictionary<IPAddress, int> mapAddr = new Dictionary<IPAddress, int>();
