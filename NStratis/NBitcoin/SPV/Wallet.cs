...
namespace NBitcoin.SPV
{
	...
	public class Wallet
	{
		private class WalletBehavior : NodeBehavior
		{
			...
			private TrackerBehavior _Tracker;
			private BroadcastHub _Broadcast;
			...
			private void _Broadcast_TransactionRejected(Transaction transaction, RejectPayload reject)
			{
				_Wallet.OnTransactionRejected(transaction, reject);
			}

			private void _Broadcast_TransactionBroadcasted(Transaction transaction)
			{
			...
			private void AttachedNode_Disconnected(Node node)
			{
		...
		private Dictionary<KeyPath, PathState> _PathStates = new Dictionary<KeyPath, PathState>();

		private int _KeyPoolSize;

		private WalletCreation _Parameters;

		private Dictionary<Script, KeyPath> _KnownScripts = new Dictionary<Script, KeyPath>();
		...
		private void _ListenerTracked_NewOperation(Tracker sender, Tracker.IOperation trackerOperation)
		{
		...
		private Tracker _ListenedTracker;
