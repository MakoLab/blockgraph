...
namespace NBitcoin.SPV
{
	...
	public class TrackerBehavior : NodeBehavior
	{
		...
		private void StartScan(object unused)
		{
		...
		private void AttachedNode_StateChanged(Protocol.Node node, Protocol.NodeState oldState)
		{
		...
		private ConcurrentDictionary<uint256, uint256> _InFlight = new ConcurrentDictionary<uint256, uint256>();

		private BoundedDictionary<uint256, MerkleBlock> _TransactionsToBlock = new BoundedDictionary<uint256, MerkleBlock>(1000);

		private void AttachedNode_MessageReceived(Protocol.Node node, Protocol.IncomingMessage message)
		{
		...
		private BlockLocator _CurrentProgress;
		private DateTimeOffset _SkipBefore;
		...
		private long _FalsePositiveCount = 0;
		...
		private double _MaximumFalsePositiveRateDifference = 0.1;
		...
		private volatile PingPayload _RunningPing;
		private ConcurrentBag<Action> _OnFilterLoaded = new ConcurrentBag<Action>();
